...
let makeAgent = function (selfParams, selfId, initialState, game) {
  ...
  let expectedUtility = dp.cache(
    function (state, action, ofAgentID, timeLeft, allMetaParams, cond, mentalSnapshot) {
      ...
      explain("EU at state " + stateToString(state) +
        " for action " + action + ", with " + timeLeft + " steps left: "
        + eu, (ofAgentID === 1 && _.isEqual(state[0], ['O',7])) || (ofAgentID === selfId && cond === undefined && globalStore.indent === 0))
      return eu
    })

  let mentalRewards = function(state) {
    let belief = belief(state)
    return computeMentalRewards(state, selfId, belief)
  }
  ...
  return {
    ...
    act,
    expectedUtility,
    // stateUtility: function(state)
    //   stateUtility = dp.cache(function (state, ofAgentID, cond, mentalSnapshot)
    belief,
    mentalRewards,
    mentalState
    ...
    // },
    // mentalState: function (state, belief) {
    //   return computeMentalState(state, belief)
    // }
  }
}