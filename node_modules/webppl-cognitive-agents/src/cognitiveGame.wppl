...
let makeCSMG = function (gameSetup, externalParams, observationsOpt) {
  ...
  /** rewards is something like [1,2,[4,2],[1,2,3]] */
  let utilityAtIndex = function(rewards, index) {
    ...
    let mentalPart = arrayConcat(rewards.slice(physicalNo, rewards.length))
    let result = mentalPart[index - physicalNo]
    info("utilityFn(): returning " + result)
    return result
  }
