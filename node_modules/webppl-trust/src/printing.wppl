...
let discreteDistributionToString = function(dist, support) {
    ...
    let actionProbPairs = map(function(elem) {
        return [Math.exp(dist.score(elem)), elem]
    }, support)
    let sorted = sort(actionProbPairs, gt, fst)
    reduceL(function(acc, probActionPair) {
        let action = probActionPair[1]
        let prob = probActionPair[0]
        let line = action + ": " + prob
