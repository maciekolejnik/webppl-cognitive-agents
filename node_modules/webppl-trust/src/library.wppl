...
/** Auxiliary functions
 * TODO: most of below should be moved into make game function
 * */
...
//   return turn + investments + returns
// }
//
...
var ifDefinedElse = function(obj, fallback) {
  return obj || fallback
}
...
/** Check if state *s1* is a predecessor of *s2* */
// let isPredecessor = function(s1, s2) {
//   if (_.isEqual(s1,s2)) return true
//   if (s1.investments.length > s2.investments.length
//       || s1.returns.length > s2.returns.length) return false
//   return isPredecessor(s1, previousState(s2))
// }
...
let makeTrustCSMG = function(gameSpecificParams) {
  ...
  // let investeeEndowment = endowments.investee
  ...
  let transitionFn = function(state, action) {
    ...
    var nextInvestments = (turn == 'investor') ?
      [action].concat(state.investments) : state.investments
    var nextReturns = (turn == 'investee') ?
   ...
   *  This is a relatively simple component,
   *
   */
  let getPhysicalRewards = function() {
    // some alternative utility below
      // var computeIncomeReward = function(state, action, role) {
      //   var transfer = transfer(state, action)[role]
      //   var endowment = endowment(state, params)[role]
      //   var income = transfer + endowment
      //   var maxPossibleIncome = maxPossibleIncome(state, role, params)
      //   console.assert(Math.abs(income) <= Math.abs(maxPossibleIncome),
      //     "Income " + income + " is bigger than calculated max possible income " + maxPossibleIncome)
      //   var incomeRatio = (maxPossibleIncome != 0) ? income / maxPossibleIncome : 0
      //   console.assert(incomeRatio >= -1 && incomeRatio <= 1,
      //     "incomeRatio out of bounds: " + incomeRatio)
      //   // return incomeRatio
      //   return income
      // }
      // return {
      //   investor: [computeIncomeReward(state, action, 'investor')],
      //   investee: [computeIncomeReward(state, action, 'investee')]
      // }
    ...
    let stateRewards = function(state) {
      let endowments = endowment(state, params)
      return {
  ...
  let getMentalStateDynamics = function(beliefAPI) {
    ...
    let updateTrustValue = function(trust, agent, state, action) {
      ...
      // let result = (transferRatio > trust) ?
    ...
    let computeTrust = function(state, belief) {
      ...
      // let beliefFn = beliefAPI.belief
      return goalCoeffExpectation(belief, 1)
    }

    return {
      estimationHeuristicArr: [ updateTrustValue ],
  ...
  let getRewardUtilityFunctions = function() {
...
let makeCSMG = function(gameSetup) {
  ...
  /** Reward structures */
  let getPhysicalRewards = gameSetup.getPhysicalRewards
  let getMentalStateDynamics = gameSetup.getMentalStateDynamics

  /** Reward Utilities */
  let getRewardUtilityFunctions = gameSetup.getRewardUtilityFunctions
  let rewardUtilityFunctions = getRewardUtilityFunctions()
...
let makeAgent = function(selfParams, selfId, initialState, game) {
  ...
  let updateBelief = function(belief, state, action) {
    var updateBeliefDiscrete = function() {
      // debug("updateBeliefDiscrete(belief, " + stateToString(state) + ", " + action + ")")
      // if (selfId == state.turn) {
      //   return belief
      // }
    ...
    */
    let updateBeliefDirichlet = function() {
      ...
      let computeActionDist = function(index) {
        ...
        */
        // let trust = computeTrust(belief)
        // let mentalSnapshot = {
        //   state,
        //   values: [trust]
        // }
        // let cond = {
        //   index, mentalSnapshot
        // }
        let cond = { index }
  ...
  */
  let expectedUtility = dp.cache(function(state, action, role, timeLeft, otherMetaParams, cond, mentalSnapshot, topLevel) {
    ...
    // if (_.isEqual(state.investments, [1]) && state.returns.length == 0 && role == 'investee') {
    //   display("actionutil for " + action + ": " + u)
    // }
    // let u = utility(state, action, role, cond)
    ...
    let eu = u + discountFactor * expectation(Infer({method: 'enumerate'}, function() {
      ...
      // if (_.isEqual(nextState.investments, [1]) && nextState.returns.length == 1 && selfId == 'investor' && role == 'investee') {
      //   display("nextTimeLeft: " + nextTimeLeft + ", util: " + nextStateUtility)
      // }
      if (nextTimeLeft == 0) {
