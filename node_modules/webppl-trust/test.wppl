...
/** agent 0*/ [[0]]
    ...
    function() {
      ...
      let makeGame = function (gameParams) {
    ...
    /** FIFTH TEST (tic tac toe pro - parent + kid with mental) */
    function() {
      let makeGame = function (gameParams) {
        /** state is an array of 'moves' where each move is a pair
         * [mark, position] (mark is one of 'X', 'O') and position
         * is an integer 0-8. most recent move is at position 0 */
        let beliefRepresentation = assertDefined(gameParams.beliefRepresentation,
          "makeGame(): gameParams.beliefRepresentation must be passed" +
          " but not found")
        ...
        let isBlunder = function(state, action) {
          ...
          let winningOpportunityLines = filter(function(line) {
            return isWinningOpportunity(line, marks[turn])
          }, allLines)
          let losingOpportunityLines = filter(function(line) {
            return isWinningOpportunity(line, marks[(turn+1)%2])
          }, allLines)
          let nextState = [action].concat(state)
          if (winningOpportunityLines.length > 0)
            return !isOver(nextState)
        ...
        /** board is a (flat) array of mark/undefined */
        let getBoard = function(state) {
          let emptyBoard = repeat(9, function() { return undefined })
          return reduceL(function(board,move) {
            return arrayReplace(board, move[1], move[0])
          }, emptyBoard, state)
        }

        let isOver = function(state) {
          return threeInARow(state, 'X') ||
            threeInARow(state, 'O') || state.length === 9
        }
        ...
        let threeInARow = function(state, mark) {
          let board = getBoard(state)
          // let rows = getRows(board)
          // let cols = getCols(board)
          // let diags = getDiagonals(board)
        ...
        let transitionFn = function (state, action) {
          ...
          assert(getBoard(state)[action[1]] === undefined,
            "move " + arrayToString(action) + " in state " +
            stateToString(state) + " invalid!")
          assert(marks[turn(state)] === action[0],
            "it's not turn of " + action[0] + " at state " +
            stateToString(state))
          let nextState = [action].concat(state)
          return Delta({v: nextState})
        }
        ...
        let physicalRewardStructure = function () {
          let stateRewards = function (state) {
            ...
            if (threeInARow(state, 'O')) return [[-1],[1]]
            return [[0],[0]]
            // let crossReward = threeInARow(state, 'X') ? 1 : 0
            // let noughtReward = threeInARow(state, 'O') ? 1 : 0
            // assert(crossReward === 0 || noughtReward === 0,
            //   "both players' rewards are nonzero")
            // return [[crossReward], [noughtReward]]
          }
          let actionRewards = function (state, action) {
            return [[0], [0]]
          }
        ...
        /** kid's satisfaction enters the picture here. */
        let mentalStateDynamics = function () {
    ...
    /** SIXTH TEST (simple trust game with fixed params) */
    function() {
      let makeGame = function () {
